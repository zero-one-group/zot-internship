...
leading_diag_prods = [
    table[i][j] * table[i+1][j+1] * table[i+2][j+2] * table[i+3][j+3]
    for i in range(len(table) - 3)
    for j in range(len(table) - 3)
]
...
other_diag_prods = [
    table[i][j] * table[i+1][j-1] * table[i+2][j-2] * table[i+3][j-3]
    for i in range(len(table) - 3)
    for j in range(len(table) - 1, 2, -1)
]

all_prods = vertical_prods + horizontal_prods + leading_diag_prods + other_diag_prods

max(prod for prod in all_prods)
