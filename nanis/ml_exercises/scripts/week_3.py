...
def accuracy(theta, x, y, threshold): 
    probability = sigmoid(x @ theta.T) >= threshold
...
def plot_data(data, xlabel, ylabel): 
    ...
    plt.scatter(get_x(admitted), get_y(admitted), marker='+', color='black', label='passed', edgecolor='black')
...
def compute_regularised_gradient(theta, lambda_value, x, y):
    ...
    intial_gradient = (1/number_of_samples) * x.T @ (predictions-y) 
